...
export const loginWithGoogle = async ({ state, effects, actions }: Context) => {
  try {
    // plugin auth flow
    // TODO: I have no clue what I am doing, pls somebody help me
    const googleData = (await GoogleAuth.signIn()).authentication;

    const responseToken = await effects.profile.loginViaThirdParty({
      token: googleData.accessToken,
    });
    let { access_token } = responseToken.data;
...
export const getSetsByUser = async ({ state, actions, effects }: Context) => {
  ...
  try {
    const response = await effects.profile.getSetsFromUser(
      state.profile.currentUser.sub
    );
