...
import {
  countPlayedByPlayer,
  genderToTaskCurrentPlayerGender,
  shuffleArray,
} from "./GameUtilities";

export const getPossibleTasks = (
  tasks: PlayTask[],
  player: Player,
  pickedTaskType: TaskType | null
) => {
  return tasks.filter(
    (task) =>
      (task.type === pickedTaskType || !pickedTaskType) &&
      (task.currentPlayerGender === TaskCurrentPlayerGender.ANYONE ||
        task.currentPlayerGender ===
          genderToTaskCurrentPlayerGender(player.gender) ||
        player.gender === Gender.DIVERS)
  );
};

export const getFillableTasks = (
  tasks: PlayTask[],
  player: Player,
  playerGenderCount: PlayerGenderCount
) => {
  ...
  const fillableTasks = tasks.filter((task) => {
    ...
    if (
      Object.values(reducedGenders).reduce((sum, value) => sum + value) -
        Object.values(task.requires).reduce((sum, value) => sum + value) <
      0
    ) {
      return false;
    }
    ...
    if (
      task.requires.male <= reducedGenders.male &&
      task.requires.female <= reducedGenders.female
    ) {
      return true;
    }
...
export const getLeastPlayedByMe = (tasks: PlayTask[], player: Player) => {
  const sortedTasks = tasks.sort(
    (a, b) =>
      countPlayedByPlayer(a.playedBy, player) -
      countPlayedByPlayer(b.playedBy, player)
  );
  return sortedTasks.filter(
    (task) =>
      countPlayedByPlayer(task.playedBy, player) ===
      countPlayedByPlayer(sortedTasks[0].playedBy, player)
  );
};
...
export const fillPlayersIntoMessage = (
  players: Player[],
  playTask: PlayTask,
  ...
  for (
    let i = 0;
    i < playTask.requires.male - playerNamesByGender.male.length;
    i++
  ) {
  ...
  for (
    let i = 0;
    i < playTask.requires.female - playerNamesByGender.female.length;
    i++
  ) {
  ...
  const message = playTask.message
    .replace(
      new RegExp(TaskPlayerGender.MALE, "g"),
      () => playerNamesByGender.male.pop()!
    )
    .replace(
      new RegExp(TaskPlayerGender.FEMALE, "g"),
      () => playerNamesByGender.female.pop()!
    )
    .replace(new RegExp(TaskPlayerGender.ANYONE, "g"), () => {
      if (
        playerNamesByGender.male.length > 0 &&
        playerNamesByGender.female.length > 0
      )
      ...
      else return playerNamesByGender.female.pop()!;
    });
...
export const countPossibleTasksForPlayer = (
  tasks: PlayTask[],
  player: Player,
  playerGenderCount: PlayerGenderCount
): number => {
